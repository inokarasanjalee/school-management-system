...
// [Vuln 5 Fix] Safe null-coalescing reads + trim
$census = trim($_POST['census'] ?? '');
$email  = trim($_POST['email']  ?? '');
$pass   = $_POST['password']    ?? '';
$conf   = $_POST['confirm']     ?? '';

// Validate census exists
...
$school_row = $check_census->get_result()->fetch_assoc();
...
if ($stmt->execute()) {
    ...
    if (sendMail($email, "Verify Your Account", $message)) {
        ...
        header("Location: ../index.php?error=Registered but email could not be sent. Please contact support.");
    }
    exit();
} else {
    error_log("Registration DB error: " . $conn->error);
